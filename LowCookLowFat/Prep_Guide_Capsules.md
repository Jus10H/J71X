# Capsule Preparation Guide

## Overview

All spices and superfoods are taken as capsules to avoid ruining the shake's taste. Using size **000 capsules** (XPRS Nutra, HPMC veggie caps), batch-fill every ~8 days using a 100-capsule filling machine.

---

## Daily Capsule Ingredients

| Ingredient | Daily Amount | Weight |
|------------|-------------|--------|
| Amla Powder | 1 tsp | ~3g |
| Ground Turmeric | 1/2 tsp | ~1.5g |
| Long Pepper (Pippali) | 1/4 tsp | ~0.6g |
| Ground Ginger | 1 tsp | ~2.5g |
| Cumin | 1 tsp | ~2.5g |
| Black Cumin (Nigella sativa) | 1/4 tsp | ~0.6g |
| Garlic Powder | 1/4 tsp | ~0.7g |
| Cayenne (90K HU) | 1/4 tsp | ~0.6g |
| **TOTAL** | | **~12g** |

**Size 000 capacity:** ~1,000mg per capsule
**Capsules per day:** ~12 capsules (4 per shake serving)

---

## 100-Capsule Batch Fill Instructions (~8 days)

### What You Need

- XPRS Nutra size 000 empty veggie capsules (100 ct)
- Capsule filling machine (size 000, 100-capsule capacity)
- Digital kitchen scale
- Blender (for mixing powders evenly)
- All 8 spice/superfood powders

### Step 1: Measure the Batch Mix

100 capsules ÷ 12 capsules/day = **~8 days per batch.**

| Ingredient | Per Batch (100 caps) |
|------------|---------------------|
| Amla Powder | 8 tsp (~25g) |
| Ground Turmeric | 4 tsp (~12.5g) |
| Long Pepper (Pippali) | 2 tsp (~5g) |
| Ground Ginger | 8 tsp (~21g) |
| Cumin | 8 tsp (~21g) |
| Black Cumin (Nigella sativa) | 2 tsp (~5g) |
| Garlic Powder | 2 tsp (~6g) |
| Cayenne (90K HU) | 2 tsp (~5g) |
| **TOTAL** | **~100g** |

### Step 2: Blend to Mix

1. Add all measured powders to a blender
2. **Pulse 5-10 times** until uniform in color (brownish-gold)
3. Do not over-blend — short pulses are enough to homogenize

Blending ensures even distribution of cayenne and turmeric throughout the mix. No hot spots.

### Step 3: Fill Capsules

1. Load the capsule machine with 100 caps (body side down, caps set aside)
2. Pour the blended powder mix over the machine
3. Use the spreader/tamper to pack powder into each capsule body
4. Tamp down firmly — a well-packed 000 cap holds ~1,000mg
5. Place the caps on top and press to close
6. **Batch total: 100 capsules (~8 days)**

### Step 4: Store

- Store filled capsules in a glass jar or resealable bag
- Keep in a cool, dry, dark place
- Use within 2 weeks (the mix contains moisture-sensitive ingredients)

---

## Daily Schedule

| Meal | Capsules | With |
|------|----------|------|
| Shake Serving 1 (morning) | 4 capsules | Take with shake + water |
| Shake Serving 2 (midday) | 4 capsules | Take with shake + water |
| Shake Serving 3 (evening) | 4 capsules | Take with shake + water |

---

## Supplement Capsules (Taken Separately)

These are purchased pre-made, NOT filled at home:

| Supplement | When | Frequency |
|------------|------|-----------|
| Algae Omega-3 (Nordic Naturals) | With food | Daily, 1-2 softgels |
| Vitamin D3 + K2 (Sports Research) | With food | Daily |
| Vitamin B12 (Bluebonnet chewable) | Empty stomach | 2-3x/week |
| Iodine (Thorne) | Any time | Daily |

*Creatine goes directly in the shake, not in capsules.*

---

## Tips

- **Turmeric stains everything.** Work on a surface you can wipe down. Wear gloves if you don't want yellow fingers.
- **Cayenne is 90K HU.** If powder gets airborne, it will irritate eyes and throat. Work in a ventilated area and avoid touching your face.
- **Pippali boosts turmeric absorption.** They're in the same mix, so this happens automatically.
- **If you miss a batch fill day**, you can dump a day's worth of the pre-mixed powder into a small glass of water and shoot it — not pleasant, but effective.

---

## Capsule Supply Math

- 12 capsules/day x 30 days = **360 capsules/month**
- 100-capsule batch = ~8 days
- **~4 batches per month**
- XPRS Nutra 100ct bag = 1 batch, reorder every 3-4 weeks (or buy 4+ bags at a time)

---

## References

- [Dr. Greger's Daily Dozen](https://nutritionfacts.org/daily-dozen/)
- [NutritionFacts.org - Turmeric](https://nutritionfacts.org/topics/turmeric/)
- [NutritionFacts.org - Pippali / Long Pepper](https://nutritionfacts.org/hnta-video/piperlongumine-a-natural-senolytic-in-the-spice-pippali-long-pepper/)
